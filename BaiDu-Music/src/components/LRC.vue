...
export default {
  ...
  mounted(){
    const LRCUrl = "/v1/restserver/ting?method=baidu.ting.song.lry&songid="+this.songid;
  ...
  watch: {
    'currentTime'(val) {
      ...
      this.keyArr.forEach((item,index) => {
        if(val >= item && val < this.keyArr[index+1]) {
          lrcDiv.style.top = -((index-2)*30)+"px"
        }
      })
      // for(var i = 0;i < this.keyArr.length; i++) {
      //   if((val > this.keyArr[i]) && (val < this.keyArr[i+1])) {
      //     console.log('+++',i)
      //     console.log('keyArr',this.keyArr)
